#include "z:\GitHub\silentarmy\param.h"
...
uint get_row(uint round, ulong xi0)
{
...
#define NUM_THREADS_PER_SLOT 4

uint get_cnt(uint row, __global uint *rowCounters, int thread_index, __local uint *cnt_array)
{
	uint cnt;
	...
	if (!thread_index) {
		...
		cnt_array[get_local_id(0) % (THRD / NUM_THREADS_PER_SLOT)] = cnt;
	}
	barrier(CLK_LOCAL_MEM_FENCE);
	if (thread_index)
		cnt = cnt_array[get_local_id(0) % (THRD / NUM_THREADS_PER_SLOT)];
	return cnt;
}
...
uint ht_store(uint round, __global char *ht, uint i,
	ulong xi0, ulong xi1, ulong xi2, ulong xi3, __global uint *rowCounters, __local uint *cnt_array)
{
	int thread_index = (round == 0) ? 0 : (get_local_id(0) / (THRD / NUM_THREADS_PER_SLOT));
	uint row = get_row(round, xi0);

	uint                cnt;
	if ((cnt = get_cnt(row, rowCounters, thread_index, cnt_array)) == 0xffffffff)
		return 1;
	__global char       *p = ht + row * NR_SLOTS * SLOT_LEN + cnt * SLOT_LEN + xi_offset_for_round(round);
	...
	if (thread_index == 0) {
		*(__global uint *)(p - 4) = i;
		...
		if (round == 1)
		{
			*(__global ulong *)(p + 0) = (xi0 >> 16) | (xi1 << (64 - 16));
			*(__global ulong *)(p + 8) = (xi1 >> 16) | (xi2 << (64 - 16));
			*(__global ulong *)(p + 16) = (xi2 >> 16) | (xi3 << (64 - 16));
		}
		else if (round == 2)
		{
			*(__global ulong *)(p + 0) = (xi0 >> 16) | (xi1 << (64 - 16));
		...
		{
			*(__global ulong *)(p + 0) = (xi0 >> 16) | (xi1 << (64 - 16));
			*(__global uint *)(p + 8) = (xi1 >> 16) | (xi2 << (64 - 16));
		}
...
uint xor_and_store(uint round, __global char *ht_dst, uint row,
	...
	__global uint *rowCounters, __local uint *cnt_array)
{
	uint i = ENCODE_INPUTS(row, slot_a, slot_b);
	int thread_index = (round == 0) ? 0 : (get_local_id(0) / (THRD / NUM_THREADS_PER_SLOT));
	...
	if (round == 0)
	{
		...
		xi1 = *(a++) ^ *(b++);
		xi2 = *a ^ *b;
		row = get_row(round, xi0);
		uint                cnt;
		if ((cnt = get_cnt(row, rowCounters, thread_index, cnt_array)) == 0xffffffff)
			return 1;
		__global char *p = ht_dst + row * NR_SLOTS * SLOT_LEN + cnt * SLOT_LEN + xi_offset_for_round(round);
		*(__global uint *)(p - 4) = i;
		xi0 = (xi0 >> 16) | (xi1 << (64 - 16));
		xi1 = (xi1 >> 16) | (xi2 << (64 - 16));
		xi2 = (xi2 >> 16) | (xi3 << (64 - 16));
	...
	} else if (round == 1)
	{
		...
		__global char *p = ht_dst + row * NR_SLOTS * SLOT_LEN + cnt * SLOT_LEN + xi_offset_for_round(round);
		switch (thread_index) {
		case 0: *(__global uint *)(p - 4) = i; break;
		case 1:	*(__global ulong *)(p + 0) = (xi0 >> 16) | (xi1 << (64 - 16)); break;
		case 2: *(__global ulong *)(p + 8) = (xi1 >> 16) | (xi2 << (64 - 16)); break;
		case 3:	*(__global ulong *)(p + 16) = (xi2 >> 16) | (xi3 << (64 - 16)); break;
		}
		return 0;
	} else if (round == 2)
	{
		...
		case 1:	*(__global ulong *)(p + 0) = (xi0 >> 16) | (xi1 << (64 - 16)); break;
		case 2: *(__global ulong *)(p + 8) = (xi1 >> 16) | (xi2 << (64 - 16)); break;
		case 3:	*(__global uint *)(p + 16) = (xi2 >> 16) | (xi3 << (64 - 16)); break;
		}
		return 0;
	}
	else if (round == 3)
	...
	else if (round == 5)
	{
		...
		if (xi0 || xi1) {
			...
			case 0: *(__global uint *)(p - 4) = i; break;
			case 1:	*(__global ulong *)(p + 0) = (xi0 >> 16) | (xi1 << (64 - 16)); break;
			case 2: *(__global uint *)(p + 8) = (xi1 >> 16); break;
			}
		}
		return 0;
	}
...
part2:
    ...
    for (uint index = get_local_id(0) % (THRD / NUM_THREADS_PER_SLOT); index < totalCollisions; index += get_local_size(0) / NUM_THREADS_PER_SLOT)
      {
...
{ \
    ...
    __local uint    collisionsNum; \
	__local uint	cnt_array[THRD / NUM_THREADS_PER_SLOT];\
    equihash_round(N, ht_src, ht_dst, debug, first_words_data, collisionsData, \
	    &collisionsNum, rowCountersSrc, rowCountersDst, THREADS_PER_ROW, cnt_array); \
}
KERNEL_ROUND(1)
KERNEL_ROUND(2)
KERNEL_ROUND(3)
